  ...
  return {
    async handleInvoicePaymentSucceeded(invoice) {
      ...
      if (rows.length > 0 && rows[0].id_susc !== null) {
        console.log(` Usuario con correo ${correo} ya tiene suscripción activa`);
        return;
      }
    ...
    async handleInvoicePaymentSucceeded(invoice) {
      if (invoice.billing_reason === 'subscription_create' || invoice.billing_reason === 'subscription_cycle') {
        try {
          ...
          // USAR EL EMAIL DEL USUARIO REAL (resetEmail)
          const userEmail = subscription.metadata.userEmail;
          const stripeEmail = customer.email; // Solo para logs
          
          if (!userEmail) {
            console.error('No se encontró userEmail en metadata de la suscripción');
            return;
          }
          
          console.log(`Procesando suscripción para usuario: ${userEmail}, email de Stripe: ${stripeEmail}`);
          ...
          const [rows] = await pool.execute('SELECT id_susc FROM musuario WHERE correo_user = ?', [userEmail]);
          if (rows.length > 0 && rows[0].id_susc !== null) {
            console.log(`Usuario con correo ${userEmail} ya tiene suscripción activa`);
            return;
          }
          ...
          // ACTUALIZAR AL USUARIO REAL (resetEmail)
          await pool.execute(
            'UPDATE musuario SET id_susc = ?, rol_user = ? WHERE correo_user = ?',
            [suscriId, 'empresa', userEmail]
          );

          console.log(`Suscripción registrada para usuario: ${userEmail}`);
